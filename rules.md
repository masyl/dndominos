...
Dungeons and Dominos (#DnDom) is a tabletop game inspired by DnD lore and the classic tile based role-playing games. You can play by only using a few dices and a set of standard dominos that you can find in any toy store.

It is a causal game you can learn in a few rounds, but can be made more complexe with extension rules for more advanced players.
...
- **Looting:** To obtain gold coins, players must find and loot each of the 6 rooms that are created when placing the double dominos.
...
- **Drop:** When a room is successfuly looted, the player gains the dropped gold coins. The amount of gold coins is equal to the sum of value of each tiles composing the room.
...
The act of exploring means that a player will places a domino, from his hand, on the table next to thee tile on which the hero is placed.
...
When placing the monsters "double dominos" (same value on both sides), you are creating a room. You must place it in a way that will form a 2 x 2 square. A player standing on one of the four tiles is considered to be inside the room.

A few other constraints apply when placing corridors:

- You can only place new corridor tiles next to tiles of equal or lesser value.
- Corridors cannot be connected to a monster domino after it has been placed.

Finally, you can choose to exchange one of your tile for another one instead of placing a domino on the table.



### Looting

In order to find gold coins by looting a room, your must first move your hero inside that room.
...
If the case that you attack with a dice value of only 1 that is not parried by a 1, you trip and fall flat on your face. Even if you had a move left, you cannot move to escape. Also, you drop one (1) gold coin, which is picked up by your victim.
